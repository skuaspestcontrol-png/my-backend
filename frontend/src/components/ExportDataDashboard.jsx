...
const buildInvoiceExportRow = ({ invoice = {}, customer = null, index = 0, contractPrefix = 'CONTRACT', serviceMeta = null } = {}) => {
  ...
  const serviceName = cleanText(
    [
      ...lineItems.map((line) => line?.serviceName || line?.itemName || line?.name || line?.description || ''),
      invoice.serviceName,
  ...
  return {
    ...
    customerType: cleanText(firstValue(invoice, ['customerType', 'customer_type'])),
    leadSource: cleanText(firstValue(invoice, ['leadSource', 'lead_source'])),
...
const moduleDefinitions = [
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'billingArea', label: 'Billing Area Name' },
      { key: 'billingCity', label: 'Billing City' },
      ...
      { key: 'shippingArea', label: 'Shipping Area Name' },
      { key: 'shippingCity', label: 'Shipping City' },
      { key: 'shippingState', label: 'Shipping State' },
      { key: 'shippingPincode', label: 'Shipping Pincode' },
  ...
  },
  {
    key: 'quotations',
    label: 'Quotations',
    description: 'Export the quotation summary fields used for customer review and record keeping.',
    ...
    mapRow: (row) => ({
      ...
      customerName: cleanText(firstValue(row, ['customerName', 'displayName'])),
      address: cleanText(firstValue(row, ['billingFullAddress', 'billingAddressText', 'billingAddress', 'address'])),
      mobileNumber: cleanText(firstValue(row, ['mobileNumber', 'mobile', 'workPhone', 'phone'])),
      serviceName: cleanText(firstValue(row, ['serviceName', 'serviceType', 'subject', 'quotationSubject'])),
      subtotalWithoutGst: cleanText(formatCurrency(firstValue(row, ['subtotalWithoutGst', 'subtotal_without_gst', 'amountWithoutGst'])), '-'),
      amountWithGst: cleanText(formatCurrency(firstValue(row, ['amountWithGst', 'grandTotal', 'totalAmount', 'total'])), '-')
    })
  },
  {
    key: 'vendors',
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'treatmentMethod', label: 'Treatment Methodology' },
    ...
    mapRow: (row) => ({
      ...
      treatmentMethod: cleanText(firstValue(row, ['treatmentMethod', 'treatment_method'])),
