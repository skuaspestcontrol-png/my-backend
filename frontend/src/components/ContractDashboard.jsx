...
const shell = {
  ...
  card: { background: '#fff', border: '1px solid rgba(15,23,42,0.08)', borderRadius: '14px', overflow: 'hidden' },
  cardTop: { padding: '10px 12px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', flexWrap: 'wrap' },
  cardTitle: { margin: 0, fontSize: '18px', fontWeight: 800, color: '#334155' },
  ...
  actionBtn: { border: '1px solid #d1d5db', background: '#fff', color: '#374151', borderRadius: '10px', minHeight: '34px', padding: '0 10px', fontSize: '12px', fontWeight: 800, cursor: 'pointer' },
  ...
  workspace: { background: '#fff', border: '1px solid rgba(15,23,42,0.08)', borderRadius: '14px', padding: '12px 14px', display: 'grid', gap: '10px' },
  ...
  detailHead: { padding: '8px 10px', background: '#f8fafc', borderBottom: '1px solid var(--color-border)', fontSize: '11px', fontWeight: 800, color: '#334155', textTransform: 'uppercase' },
  detailTable: { width: '100%', borderCollapse: 'collapse' },
  detailTh: { fontSize: '10px', fontWeight: 800, color: '#64748b', textAlign: 'left', padding: '7px 10px', borderBottom: '1px solid var(--color-border)', background: '#fff' },
  detailTd: { fontSize: '12px', color: '#1f2937', padding: '7px 10px', borderBottom: '1px solid #f1f5f9' },
...
export default function ContractDashboard() {
  ...
  const sortedContracts = useMemo(() => {
    if (!sortConfig.key) return filteredContracts;

    const getValue = (row) => {
      if (sortConfig.key === 'contractNo') return String(row.contractNo || '').toLowerCase();
      if (sortConfig.key === 'customer') return String(row.customer || '').toLowerCase();
      if (sortConfig.key === 'property') return String(row.property || '').toLowerCase();
      if (sortConfig.key === 'duration') return row.startDate || '';
      if (sortConfig.key === 'services') return Number(row.services || 0);
      if (sortConfig.key === 'status') return String(row.status || '').toLowerCase();
      if (sortConfig.key === 'total') return Number(row.total || 0);
      if (sortConfig.key === 'paid') return Number(row.paid || 0);
      if (sortConfig.key === 'due') return Number(row.due || 0);
      return '';
    };
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {sortedContracts.map((row, index) => {
            ...
            const statusTone = statusStyles[row.status] || statusStyles.Active;
            return (
              <tr key={row.id} onClick={() => setSelectedContractId(row.id)}>
                ...
                {visibleColumns.status ? <td style={shell.td}>
                  <span style={{ ...shell.statusPill, ...statusTone }}>{row.status.toUpperCase()}</span>
                </td> : null}
