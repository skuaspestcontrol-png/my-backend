...
function registerCustomerDedupModule({ app, readJsonFile, files, mysql = {}, uploadMiddleware = null }) {
  ...
  const createCustomerRecord = (payload = {}) => {
    ...
    const customer = {
      ...
      city: normalizeText(payload.billingCity || payload.city || billingState),
      state: billingState,
    ...
    const candidates = customers
      .map((entry) => {
        ...
        return {
          ...
          segment: entry.segment || 'Residential',
          companyName: entry.companyName || '',
          contactPersonName: entry.contactPersonName || '',
          position: entry.position || '',
          positionCustom: entry.positionCustom || '',
          ...
          billingArea: entry.billingArea || entry.area || '',
          billingState: entry.billingState || entry.state || entry.placeOfSupply || '',
          billingPincode: entry.billingPincode || entry.pincode || '',
          shippingAddress: entry.shippingAddress || '',
          shippingArea: entry.shippingArea || '',
          shippingState: entry.shippingState || '',
          shippingPincode: entry.shippingPincode || '',
          area: entry.area || entry.billingArea || '',
